...
// 思路：同样可以使用BFS来遍历，不过需要有标志来记录换行。
//       对每层节点进行计数，在判断子节点存在与否时就进行对层次计数。
//       通过两个变量来实现，to_print 当前层还需要打印的数量； new_line
//       下一层的数量的计数。它们协同合作更新。
...
void printTreeBFSline(BinaryTreeNode *head) {
    if(head == nullptr)
        return;
